...
class VisionTransformerCE(VisionTransformer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        if self.prompt_type in ['vipt_shaw', 'vipt_deep']:
            ...
            gate_logit = (torch.ones(11))
            self.gate = nn.Parameter(gate_logit)
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i >= 1:
                if self.prompt_type in ['vipt_deep']:
                    ...
                    # re-conduct CE
                    # x = x_ori*g + candidate_elimination_prompt(x, global_index_t.shape[1], global_index_s)*(1-g)
                    #x = x_ori + candidate_elimination_prompt(x, global_index_t.shape[1], global_index_s)
                    g = self.gate[i].sigmoid()
                    x = x*(1-g) + x_ori*g
