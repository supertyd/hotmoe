...
class HotMoEDistillActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        # compute losses
        loss, status = self.compute_losses(out_dict, out_dict_teacher, data)
    ...
    def compute_losses(self, pred_dict, pred_dict_teacher ,gt_dict, return_status=True):
        # gt gaussian map
        ...
        pred_boxes_vec_teacher = box_cxcywh_to_xyxy(pred_boxes_teacher).view(-1, 4)
        ...
        try:
            _, iou_teacher = self.objective['giou'](pred_boxes_vec_teacher, gt_boxes_vec)  # (BN,4) (BN,4)
        except:
        ...
        loss = self.loss_weight['giou'] * giou_loss + self.loss_weight['l1'] * l1_loss + self.loss_weight['focal'] * location_loss + \
               self.loss_weight['feat'] * distill_loss_logits
        if return_status:
            # status for log
    ...
    def compute_losses_distill(self, pred_dict, pred_dict_teacher):
        ...
        #dis_loss_logits = self.distill_logits_loss(F.log_softmax(pred_dict['score_map'].view(pred_dict_teacher["score_map"].shape[0], -1), dim=1), pred_dict_teacher['score_map'].view(pred_dict_teacher["score_map"].shape[0], -1))
        dis_loss_logits = self.objective["focal"](
            pred_dict['score_map'],
            pred_dict_teacher['score_map'])
