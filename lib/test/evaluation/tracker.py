...
class Tracker:
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        if self.dataset_name in ['depthtrack_rgbd', 'depthtrack_rgbd_train', 'cdtb', 'votrgbd22_rgbd']:
            ...
            image = self._read_image(seq.frames[0], seq.frames_hsi[0])
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            if self.dataset_name in ['depthtrack_rgbd', 'depthtrack_rgbd_train', 'cdtb', 'votrgbd22_rgbd']:
                ...
                image = self._read_image(frame_path,frame_path.replace("HSI-VIS-FalseColor","HSI-VIS").replace("jpg","png"))
    ...
    def get_parameters(self,epoch = 300):
        ...
        params = param_module.parameters(self.parameter_name,epoch=300)
        return params

    def _read_image(self, image_file: str, image_hsi: str):
        if isinstance(image_file, str) and isinstance(image_hsi, str):
            img = cv.cvtColor(cv.imread(image_file), cv.COLOR_BGR2RGB)
            hsi_img = cv.imread(image_hsi, -1)
            hsi_img_norm = X2Cube(hsi_img)
            return np.concatenate((img, hsi_img_norm), axis=2)
