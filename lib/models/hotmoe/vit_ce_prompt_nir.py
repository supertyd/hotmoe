    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        if self.prompt_type in ['vipt_shaw', 'vipt_deep']:
            ...
            for i in range(block_nums):
                prompt_blocks_nir.append(Prompt_block(inplanes=embed_dim, hide_channel=8, smooth=True))
            self.prompt_nir_blocks = nn.Sequential(*prompt_blocks_nir)
            prompt_norms_nir = []
            for i in range(block_nums):
                prompt_norms_nir.append(norm_layer(embed_dim))
            self.prompt_nir_norms = nn.Sequential(*prompt_norms_nir)
...
def vit_base_patch16_224_ce_prompt_nir(pretrained=False, **kwargs):
